...
class Level_2(LevelClass):

    def __init__(self):
        self.level_name = 'level_2'
        ...
        self.enemy_1 = EnemyRedSmall(200, 100, 40, 10, "PRZECIWNIK", 330, 40, 300)
        self.enemy_2 = EnemyRedSmall(400, 200, 440, 10, "PRZECIWNIK", 730, 40, 300)
        self.enemy_bullets = [] 
        self.state = 'game'
    ...
    def draw(self, win):
        ...
        for enemy_bullet in self.enemy_bullets:
            ...
            if enemy_bullet.hit(self.player, win):
                self.enemy_bullets.remove(enemy_bullet)
                self.player.hp -= 20
        self.draw_regural_things(win, [self.enemy_1, self.enemy_2], normal_check_win_or_lose=False)
    ...
    def check_win_or_lose(self, win):
        ...
        if self.enemy_1.hp <= 0 and self.enemy_2.hp <=0:
            #print("WYGRANA")
            self.state = 'won'
            ...
            if self.is_star_data == False:
                ...
                self.star_data['level_2']["star_1"] = True
                if self.player.hp == 200:
                    self.star_data['level_2']["star_2"] = True
                if not self.lost_bullet:
                    self.star_data['level_2']["star_3"] = True
